        ...
        <el-container>
            ...
            <el-main>
                    ...
                    <div v-for="item in conversationList" :key="item.Id" :class="item.img == false ? 'chat_my_left' : 'chat_my_left2'" >
                        <div v-show="item.Types == 1">
                            ...
                            <div class="chat_details_sentence">
                               ...
                               <img class="img_chat" v-if="item.img" :src="item.Message" alt="">
                            </div>
                        </div>
                ...
                <div class="chat_sendout_box">
                    <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="chat_sendout" class="chat_sendout_ipt"></el-input>
 ...
 data() {
  ...
  created() {
      const _this = this;
        ...
        _this.demoChatHubProxy.on("addContosoChatMessageToPage", function ( message) {
        ...
        //接收私聊消息
         _this.demoChatHubProxy.on("remindMsg", function (sendId,sengName, message,img) {
            console.log('发送方id：'+sendId+'名字:'+sengName+'消息内容：' + message+img);
            let datas = {
               CustomerId:sendId,
               CustomerName:sengName,
               Message:message,
               Types:1,
               CreateTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
               img:img
           } 
